...
export const useWillContract = () => {
  ...
  const claimWill = async (owner: string, index: number) => {
    // If no owner provided, default to current address, though logic suggests 
    // you usually claim from someone else's will. 
    ...
    try {
      ...
      await writeContractAsync({
        ...
        args: [targetOwner as `0x${string}`, BigInt(index)],
      })
    } catch (err) {
